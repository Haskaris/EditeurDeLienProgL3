	...
	if (argc != 4) {
		...
		if (elfFile == NULL) {
			...
			if (memcmp(header.e_ident, ELFMAG, SELFMAG) == 0) {
				switch(choix){
					case 1:
						affichage_Entete_Fichier_ELF(header, bigEndian);
						break;
					case 2:
						affichage_Table_Sections(elfFile, header, bigEndian);
						break;
					case 3:
						affichage_Contenu_Section(elfFile, header, bigEndian, numSection);
						break;
					case 4:
						affichage_Table_Des_Symbole(elfFile, header, bigEndian);
						break;
					case 5:
						affichage_Table_Reimplantation(elfFile, header, bigEndian);
						break;
					default:
