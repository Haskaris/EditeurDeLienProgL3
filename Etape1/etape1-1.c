...
void affichage_Entete_Fichier_ELF(Elf32_Ehdr header, int bigEndian) {
	...
	printf("\n  Début des en-tête de programme:\t%u",
		byteshift32(header.e_phoff, bigEndian));
	printf("\n  Début des en-tête de section:\t\t%u",
		byteshift32(header.e_shoff, bigEndian));
	printf("\n  Fanions:\t\t\t\t0x%01x",
		byteshift32(header.e_flags, bigEndian));
	printf("\n  Taille de cet en-tête:\t\t%d (octets)",
		byteshift16(header.e_ehsize, bigEndian));
	printf("\n  Taille de l'en-tête du programme:\t%d (octets)",
		byteshift16(header.e_phentsize, bigEndian));
	printf("\n  Nombre d'en-tête du programme:\t%d",
		byteshift16(header.e_phnum, bigEndian));
	printf("\n  Taille des en-têtes de section:\t%d (octets)",
		byteshift16(header.e_shentsize, bigEndian));
	printf("\n  Nombre d'en-têtes de section:\t\t%d",
		byteshift16(header.e_shnum, bigEndian));
