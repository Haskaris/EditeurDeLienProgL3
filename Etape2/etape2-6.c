...
int init_header(Elf32_Ehdr* headerOutput, Elf32_Ehdr header1, Elf32_Ehdr header2) {
	if (header1.e_type == header2.e_type) {
		...
		//Pas le même type, pas de fusion
		return 1;
	}
}
...
int fusion_section(FILE * elfFile1, FILE * elfFile2, FILE * outputFile) {
	...
	if (init_header(&headerOutput, header1, header2)) {
		printf("Erreur : Les fichiers ne sont pas du même type.\n");
		return 1;
	}

	int j = 0;
	if (section1.sh_type == section2.sh_type) {
		//Même type alors on peut potentiellement fusionner
	...
	} else if (header1.e_type == PROGBITS && header2.e_type != PROGBITS) {
		//Ajouter 1
	} else if (header1.e_type != PROGBITS && header2.e_type == PROGBITS) {
		//Ajouter 2
	}
	...
	for (int i=0; i<header1.e_shnum; i++) {
		...
		for (int j=0; j<header2.e_shnum; j++) {
			...
			if (!strcmp(nom_section1, nom_section2) && header1.e_type == PROGBITS && header2.e_type == PROGBITS) {
			...
			else if (header2.e_type == PROGBITS) {
